...
export function normalizeCampaignInput(body = {}, { requireContent = true } = {}) {
  ...
  return {
    data: {
      ...
      subject,
      preheader: body.preheader ? String(body.preheader).trim() : '',
      fromName: body.fromName ? String(body.fromName).trim() : '',
      fromEmail: body.fromEmail ? String(body.fromEmail).trim() : '',
      replyTo: body.replyTo ? String(body.replyTo).trim() : '',
      htmlBody,
      emailDesign: body.emailDesign && typeof body.emailDesign === 'object' ? body.emailDesign : null,
      textBody,
...
export function serializeCampaign(doc) {
  ...
  return {
    ...
    emailDesign: data.emailDesign || null,
