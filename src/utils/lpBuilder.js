...
export function buildFullLP(name, niche, offer, tagline, c, isAR, variant, price = 27, aiData = {}) {
  ...
  const sec7 = S(
    ...
    <div class="video-box" style="background:${card};border:1px solid ${border};border-radius:20px;padding:50px 30px;max-width:680px;margin:0 auto;cursor:pointer;transition:transform .2s" onmouseover="this.style.transform='scale(1.01)'" onmouseout="this.style.transform='none'" onclick="alert('${
  ...
  const sec8 = S(
    ...
    <div class="cta-box" style="background:linear-gradient(135deg,${c}22,${c}44);border-radius:20px;padding:36px;text-align:center;align-self:start">
...
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:${font};background:${bg};color:${fg};direction:${R};overflow-x:hidden}nav{position:sticky;top:0;z-index:999;background:${bg}ee;backdrop-filter:blur(12px);padding:14px 24px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid ${border}}nav .logo{font-size:18px;font-weight:800;color:${c};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:60%}nav ul{display:flex;gap:20px;list-style:none}nav ul li a{font-size:13px;color:${sub};text-decoration:none;font-weight:500;transition:color .2s}nav ul li a:hover{color:${c}}.proof-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:20px;text-align:center}.about-grid{display:grid;grid-template-columns:1fr 1.5fr;gap:40px;align-items:center}.features-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px}.testimonials-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px}.plans-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px;align-items:stretch}.faq-grid{display:grid;grid-template-columns:1fr 1fr;gap:40px}@media(max-width:768px){nav{padding:10px 14px!important}nav .logo{font-size:15px!important}nav button{padding:7px 14px!important;font-size:12px!important}nav ul{display:none}section{padding:45px 16px!important}.proof-grid{grid-template-columns:repeat(2,1fr)!important;gap:12px!important}.about-grid{grid-template-columns:1fr!important;gap:24px!important;text-align:center!important}.about-grid h2,.about-grid p,.faq-grid h2,.faq-grid p{text-align:center!important}.features-grid{grid-template-columns:1fr!important;gap:16px!important}.testimonials-grid{grid-template-columns:1fr!important;gap:16px!important}.plans-grid{grid-template-columns:1fr!important;gap:24px!important}.plans-grid > div{transform:none!important}.faq-grid{grid-template-columns:1fr!important;gap:30px!important}div[style*="font-size:clamp(32px"]{font-size:28px!important}section button{padding:12px 24px!important;font-size:15px!important}.plans-grid button{padding:12px 20px!important;font-size:14px!important}.video-box{padding:30px 16px!important}.cta-box{padding:24px 16px!important;align-self:stretch!important}}@media(max-width:480px){.proof-grid{grid-template-columns:1fr!important}}</style>
</head><body>
...
<footer style="background:${card};padding:30px 20px;text-align:center;border-top:1px solid ${border}"><div style="font-size:15px;font-weight:700;color:${fg};margin-bottom:6px">${name}</div><div style="font-size:12px;color:${sub}">${niche} ${
  ...
  } · © 2026 ${name}. ${isAR ? 'جميع الحقوق محفوظة.' : 'All rights reserved.'}</div></footer>
</body></html>`;
}
